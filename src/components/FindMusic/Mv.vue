  ...
  <div class="Mv">
    <div class="M_l">
      <div class="title">
        ...
        <span class="name">
          <template v-for="(item, index) of stateList.artists" :key="index">
            <router-link :to="'/artist?id=' + item.id">{{
              item.name
            }}</router-link>
            <span v-if="index != stateList.artists.length - 1"> / </span>
          </template>
        </span>
      </div>
      <div class="playmv">
      ...
      <div class="btns">
        ...
        <a href="javascript:;" class="btn3"
          ><span>({{ stateList.subCount }})</span></a
        >
    ...
    <div class="M_r">
      ...
      <div class="mv_txt">
        <p>发布时间：{{stateList.publishTime}}</p>
        <p>播放次数：{{NumberW(stateList.playCount)}}次</p>
